...
//        //登陆成功后用户名密码存储userdefult
//        [userDefaults setObject:username forKey:@"user_id"];
//        [userDefaults setObject:password forKey:@"psw"];
//        [userDefaults synchronize];
//
//        //跳转主界面
//        ZHTabBarViewController *maincorl = [[ZHTabBarViewController alloc] init];
//        //[self.navigationController pushViewController:maincorl animated:YES];
//        [self presentViewController:maincorl animated:YES completion:^{
//            ZHLog(@"---");
//        }];
//    }
}
...
{
    ZHUserInfo *userInfo = [[ZHUserInfo alloc] init];
    NSLog(@"signupAction");
    ...
    params[@"shop_id"] = userInfo.userName;
    params[@"psw"] = userInfo.passWord;
    mgr.requestSerializer = [AFJSONRequestSerializer serializer];
    mgr.responseSerializer = [AFJSONResponseSerializer serializer];
    //mgr.responseSerializer = [AFJSONResponseSerializer new];
    mgr.responseSerializer.acceptableContentTypes = [NSSet setWithObjects:@"text/plain", nil];
    [mgr GET:@"http://localhost:8123/Wise_Server/new_SellerLogin" parameters:params progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        ZHShopInfoModel *shopInfo = [[ZHShopInfoModel alloc] initWithDict:responseObject];
        
        if (shopInfo.state == 0) {
            [MBProgressHUD showError:@"用户不存在"];
        }else if (shopInfo.state == 2) {
            [MBProgressHUD showError:@"用户密码错误"];
        }else if (shopInfo.state == 1) {
